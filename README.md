...
Although the actions are created as class instances, internally, the `prototype` is set to `null`, so they are compatible with `reactjs/redux`, as they are plain objects.
